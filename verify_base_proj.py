...
def verify_and_benchmark(M, batch_dims=(), iters=100, warmup=10):
    device = torch.device('cuda')
    
    # Input shapes
    input_shape = batch_dims + (M, 2)
    
    # Initialize inputs
    input_tensor = torch.randn(input_shape, device=device)
    weight = torch.randn(1, 2, device=device)
    ...
    def cuda_custom(x, w, b):
        return crina_cuda.base_proj_forward(x, w, b)
...
if __name__ == "__main__":
    ...
    # Standard 2D case
    print("\nCase 1: Standard 2D [256, 2] x [2]")
    verify_and_benchmark(M=256)
    
    # Broadcasting case (3D)
    print("\nCase 2: 3D Broadcasting [8, 128, 2] x [2]")
    verify_and_benchmark(M=128, batch_dims=(8,))
    
    # Small shapes (Stress tiling boundaries)
    print("\nCase 3: Small Bounds [7, 2]")
    verify_and_benchmark(M=7)
    
    # Large case
    print("\nCase 4: Large [1024, 2]")
    verify_and_benchmark(M=1024)
